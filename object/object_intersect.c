...
/*   Updated: 2025/08/28 20:13:23 by jisokim2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	hit_cylinder_side(t_cylinder *cylinder, t_ray *ray, t_hit *hit)
{
	...
	{
		//printf("Hit point is outside cylinder height.\n");
		return (false);
	}
...
bool	hit_cylinder_cap(t_cylinder *cylinder, t_vec3 cap_center, t_ray *ray, t_hit *hit, t_vec3 cap_normal)
{
	...
	if (fabs(check) < EPSILON * 10)
	{
		//printf("Ray is parallel to cap plane.\n");
		return (false);
	}
	...
	// // 플래그 설정
	// if (vec3_dot(cap_normal, cylinder->axis) > 0)
